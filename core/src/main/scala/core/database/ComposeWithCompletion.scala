...
trait ComposeWithCompletion[F[_], Out] {
  ...
  def apply[In](resource: In, f: In => F[Out])(onComplete: In => Unit)(implicit ec: ExecutionContext): Outer
}
...
object ComposeWithCompletion extends LowPriorityCompose {
  ...
  implicit def futureOut[A]: Aux[Future, A, A] = new ComposeWithCompletion[Future, A] {
    type Outer = Future[A]

    def apply[In](resource: In, f: In => Future[A])(onComplete: In => Unit)(implicit ec: ExecutionContext): Outer =
      f(resource).andThen {
