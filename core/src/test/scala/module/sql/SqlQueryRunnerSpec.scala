...
import com.zengularity.querymonad.test.core.module.sql.models.{Material, Professor}
...
object SqlQueryRunnerSpec extends Specification {
  ...
  "SqlQueryRunner" should {
    ...
    "retrieve material with id 1" in {
      ...
      val result = Await.result(resF, 1.minute)
      result aka "material" mustEqual Material(1, "Computer Science", 20, "Beginner")
    }
    ...
    "retrieve professor with id 1 and his material" in {
      val handler = AcolyteDSL.handleStatement.withQueryDetection("^SELECT *").withQueryHandler {
      ...
      val resF   = runner(query)
      val result = Await.result(resF, 1.minute)
      result aka "professor and material" mustEqual Tuple2(Professor(1, "John Doe", 35, 1), Material(1, "Computer Science", 20, "Beginner"))
    }
    ...
    "not retrieve professor with id 1 and no material" in {
      ...
      val query  = for {
        ...
        material <- SqlQueryO.fromQuery(Material.fetchMaterial(professor.material))
      } yield (professor, material)
      val resF   = runner(query)
      val result = Await.result(resF, 1.minute)
